...
us_weight_data <- 
  ...
  select(
    box_name, participant, visit, visit_code,
...
tmp <- 
  ...
  select(location, participant, visit_code, weight, volume) |>
  distinct() |> 
  filter(location == "US") |> 
...
tmp_coldata <- 
  ...
  as.data.frame() |> 
  as_tibble() |> 
  select(sample_id, location, participant, visit_code, weight_manifest, volume_manifest, weight, volume) |>
  left_join(
    us_weight_data |> 
      select(participant, visit_code, softcup_weight, softcup_volume_with_saline_imp, volume_of_saline_added)
  ) |> 
  mutate(
    weight = 
      case_when(
...
aggregate_se <- function(se){
  ...
  value_type_assay <- 
    ...
    arrange(sample_id, .feature, value_type) |> 
    group_by(sample_id, .feature, value_type) |> 
  ...
  value_type_summary_assay <- 
    value_type_summary_assay |> 
    full_join(
       se |> as_tibble() |> select(.feature, sample_id) |> distinct(),
      by = join_by(.feature, sample_id)
    ) |>
    mutate(
      value_type_summary = value_type_summary |> str_replace_na("Failed sample")
    ) |>
    pivot_wider(names_from = sample_id, values_from = value_type_summary) |> 
    as.data.frame() |> 
    column_to_rownames(".feature")
  ...
  sd_log2_conc_assay <- 
    ...
    pivot_wider(names_from = sample_id, values_from = sd) |>
    as.data.frame() |>
    column_to_rownames(".feature")
  ...
  coldata_manifest <- 
    ...
    as_tibble() |> 
    select(
      sample_id, location, participant, visit_code, visit, 
      weight, volume, approx_volume_based_on_aliquot_repos, 
  ...
  coldata <- 
    coldata_manifest |> 
    left_join(coldata_agg, by = join_by(sample_id)) |> 
    mutate(rownames = sample_id) |> 
    as.data.frame() |> 
    column_to_rownames("rownames") 
    
  f <- se@NAMES
  s <- coldata$sample_id |> sort()
...
selection <- 
  ...
  group_by(participant) |> mutate(n_visits = visit |> n_distinct()) |> ungroup() |> 
...
selection |> 
  ggplot() +
  aes(x = visit |> factor(), y = obs_conc_imp_log2, col = participant) +
...
selection |> 
  ...
  ungroup() |> 
  ggplot() +
  aes(x = visit |> factor(), y = participant, fill = z) +
  geom_tile() +
