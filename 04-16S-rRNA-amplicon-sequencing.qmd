...
```{r}
rm(counts, ASV_stats, singletons_to_check, samples_to_check, props_to_check, props_to_check_long)
```
...
make_se_from_ps <- function(ps) {
  ...
  se_rowdata <- 
    ...
    group_by(Species) |> 
    mutate(ASV_id = str_c(Species, " (ASV", ASV_order |> str_pad(width = 6, pad = "0"),")")) |> 
    ungroup() |> 
  ...
  rowData_dictionary <- 
    ...
    mutate(
      ...
      case_when(
        ...
        name == "asv_order" ~ "ASV order as provided in the original `phyloseq` object",
        name == "asv_sequence" ~ "ASV sequence",
        name == "last_available_taxonomic_level" ~ "Last available taxonomic level for the ASV",
        TRUE ~ "???"
      )
    )
...
se_16S_ASV$pid <- 
  ifelse(
    !(se_16S_ASV$sample_type %in% c("Clinical sample", "Clinical sample (other study)")), 
    NA_character_, 
    se_16S_ASV$pid
    )
...
se_16S_ASV@colData$resequenced[se_16S_ASV@colData$sample_id %in% replicates$sample_id[replicates$pool != "pool10"]] <- "Yes"
se_16S_ASV@colData$resequenced[se_16S_ASV@colData$sample_id %in% replicates$sample_id[replicates$pool == "pool10"]] <- "2nd sequencing"
...
empty_cols <- colData(se_16S_ASV) |> apply(2, function(x) {is.na(x) |> all()}) |> which() |> names() 
if (length(empty_cols) > 0) {
  warning(str_c("The following columns in the colData are empty and will be removed: ", str_c(empty_cols, collapse = ", ")))
}
unique_values_cols <- (apply(colData(se_16S_ASV), 2, function(x) {unique(x) |> length()}) == 1) |> which() |> names()
if (length(unique_values_cols) > 0) {
  warning(str_c("The following columns in the colData have only one unique value and will be removed:\n\t", str_c(unique_values_cols, collapse = ",\n\t"))) 
}
...
ordered_cols <- 
  unique(
    c(
      "uid", "pid", "visit_code", "sample_type", "control_type", "site",
      "ext_lib_plate_type", "ext_lib_plate_nb", 
      "total_counts", "resequenced",
      "sample_id_ps", "sample_id_barcode", "patient_id_ps", "visit_code_ps",
      "specimen_type", "description", "original_mgh_aliquot_unique_identifier", 
      "pool", "pool10", "researcher", "well_id", "volume_pooled_u_l",
...
se_16S_ASV@metadata$dictionary <- 
  ...
  mutate(
    ...
    description = 
      case_when(
        original_name == "sampleID" ~ "Original sample ID from the 16S data phyloseq object",
        original_name == "Sample.ID.Barcode" ~ "Concatenation of the swab barcode and the sample ID from the 16S data phyloseq object",
        original_name == "Patient.ID" ~ "Patient ID as provided in the 16S data phyloseq object",
        original_name == "Visit.code" ~ "Visit code as provided in the 16S data phyloseq object",
        ...
        name == "sample_type" ~ "Type of sample (clinical sample, positive control, negative control, etc.)",
        ...
        name == "resequenced" ~ "Whether the sample was resequenced due to low read count or other issues",
...
tmp <- 
  ...
  mutate(
    n = n(),
    new_colnames = str_c(uid, ifelse((n > 1) & (row_number() > 1), str_c("_r", row_number()), ""))
  ) |> 
  ungroup() |> 
  arrange(i)
...
aggregate_se <- function(se, agg_var = "aggregating_variable", assays){
  ...
  if (!any("taxon_id" %in% colnames(rowData(se)))) {
    warning("No `taxon_id` variable in the rowData. A `taxon_id` variable will be created with the same values as the `agg_var` variable.")
    rowData(se)$taxon_id <- NA_character_
  }
  if (!any("taxon_label" %in% colnames(rowData(se)))) {
    warning("No `taxon_label` variable in the rowData. A `taxon_label` variable will be created with the same values as the `agg_var` variable.")
  ...
  aggregated_row_data <-
    ...
    mutate(
      ...
      taxon_label =
        case_when(
          is.na(taxon_label) ~ new_feature |> str_replace_all("_", " ") |> str_to_sentence() |> str_c(" (aggregated)"),
          TRUE ~ taxon_label
        )
    ) |>
    arrange(i) |>
  ...
  if (length(j) > 0) {
    warning(str_c("The following variables in the rowData are all NA after aggregation and will be removed: ", str_c(colnames(aggregated_row_data)[j], collapse = ", ")))
    aggregated_row_data <- aggregated_row_data |> select(-all_of(colnames(aggregated_row_data)[j]))
  }
  
  aggregated_row_data <- 
    aggregated_row_data |> 
    select(taxon_id, taxon_label, domain:species, last_available_taxonomic_level, n_aggregated_features, everything())
...
se_16S_species <- 
  aggregate_se(
    se_16S_ASV, agg_var = "species", assays = c("counts", "rel_ab")
    )
...
tmp |> 
  ggplot() +
  aes(x = pool |> parse_number() |> factor(), y = rel_ab, 
      fill = .feature, alpha = total_counts |> log10()) +
  geom_col() +
  scale_fill_manual(
    str_c("Top ", tmp$.feature |> unique() |> length(), " taxa (based on max rel. ab.)"), 
    breaks = tmp$.feature |> unique(), 
...
if (selected_samples$uid |> str_detect("_r") |> any()) {
  warning("There are still some duplicated samples in the selected samples. Please check the `selected_samples` object.")
}
...
saveRDS(
  ...
  str_c(
    get_01_output_dir(),  
    "04_se_16S_ASV_all_", today() |> str_remove_all("-"), ".rds"
    )
  )
...
saveRDS(
  ...
  str_c(
    get_01_output_dir(),  
    "04_se_16S_ASV_", today() |> str_remove_all("-"), ".rds"
    )
  )
...
saveRDS(
  ...
  str_c(
    get_01_output_dir(),  
    "04_se_16S_agg_all_", today() |> str_remove_all("-"), ".rds"
    )
  )
