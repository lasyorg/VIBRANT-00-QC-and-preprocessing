...
# deux participantes MGH sont présentent alors qu'elle ne devrait pas (et n'ont que des missing) --> on les supprime
...
participants_crf[["crf101"]] <- 
  ...
  group_by(pid) |>
  summarise(
    # Pour toutes les variables sauf edu et income_sources
    across(
      .cols = !any_of(c("edu", "income_sources")),
      .fns = ~ dplyr::first(.x)
    ),
    
    # Pour edu : prendre le niveau le plus élevé
    edu = {
      if (all(is.na(edu))) {
        NA_character_
      } else {
    ...
    # Pour income_sources : prendre la réponse la plus complète
    income_sources = {
