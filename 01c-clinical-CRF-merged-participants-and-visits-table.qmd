...
The purpose of this document is to create, from the data prepared in previous documents, two tables collating participant-invariant (`participant_crfs_merged`) or time-varying (`visits_crfs_merged`) variables collected across CRFs. Specifically, this document creates

- `participant_crfs_merged`: a table with one row per `pid` containing participant-invariant data collected from CRFs that were filled out only once per participant.

- `visits_crfs_merged` : a table with one row per `pid` and `visit_code` with harmonized data from CRFs that were filled out at multiple visits.
...
The participant-invariant variables were expected to be found in the following CRFs, which were supposed to be filled only once per participant:
...
Data related to some of those CRFs contained duplicate `pid` values. It appears that some CRFs were filled at several visits and some are in long format. 

We need to transform them such that we have a single entry per participant before creating the `participants_crfs_merged` table.
...
## Harmonizing CRFs data for participants with multiple entries

Below, we create a function that, for each CRF in the participants table, compares the rows corresponding to duplicate `pid`s.  
...
Additionally, the `visit_code` and `study_day` variables are removed from these CRFs.
...
# 2 MGH participants are found in the data but should not (CRF101 should only contain data from CAP participants)
# These two MGH participants have empty entries in CRF101, so we remove these entries
participants_crf[["crf101"]] <-
  participants_crf[["crf101"]] |>
  filter(!str_starts(pid, "06810"))
...
participants_crf[["crf101"]] <-
  ...
  group_by(pid) |>
  summarise(
    # For all variables except edu & income_sources, we take the first value
    across(
      .cols = !any_of(c("edu", "income_sources")),
      .fns = ~ dplyr::first(.x)
    ),

    # For edu : we take the highest values
    edu = {
      if (all(is.na(edu))) {
        NA_character_
      } else {
        edu_non_na <- edu[!is.na(edu)]
        matches <- match(edu_non_na, education_levels)
        edu_non_na[which.max(matches)]
      }
    },

    # For income_sources : we take the most complete entry
    income_sources = {
      if (all(is.na(income_sources))) {
        NA_character_
      } else {
        inc_non_na <- income_sources[!is.na(income_sources)]
        counts <- stringr::str_count(inc_non_na, ",") + 1
        inc_non_na[which.max(counts)]
      }
    },
...
worry_cols <- names(participants_crf[["crf2"]])[str_detect(
  names(participants_crf[["crf2"]]),
  "^worry_"
)]
...
participants_crf[["crf5"]] <-
  ...
  group_by(pid) |>
  summarise(
    no_pre_existing_condition = if (length(unique(no_pre_existing_condition)) == 1) {
      unique(no_pre_existing_condition)
    } else {
      paste(unique(no_pre_existing_condition), collapse = " & ")
    },
      ...
      if (length(unique(ongoing_enrollment)) == 1) {
        ...
        paste(
          sapply(1:n(), function(i) {
            if (ongoing_enrollment[i] != ongoing_enrollment[1]) {
              paste0(ongoing_enrollment[i], " (", diagnosis[i], ")")
            } else {
              ongoing_enrollment[i]
            }
          }),
          collapse = " & "
        ) |>
    ...
    ongoing_at_exit = if (length(unique(ongoing_at_exit)) == 1) {
      unique(ongoing_at_exit)
    } else {
      paste(
        sapply(1:n(), function(i) {
          if (ongoing_at_exit[i] != ongoing_at_exit[1]) {
            paste0(ongoing_at_exit[i], " (", diagnosis[i], ")")
          } else {
            ongoing_at_exit[i]
          }
        }),
...
participant_crfs_merged <-
  ...
  map(
    function(df) {
      if ("dfseq" %in% names(df)) {
        df |> select(-any_of(c("uid", "visit_code", "dfseq", "study_day", "vdate_fixed")))
      } else {
        df
      }
    }
...
The created table has `r nrow(participant_crfs_merged)` entries (unique participants) and `r ncol(participant_crfs_merged)` variables:
...
visdat::vis_dat(participant_crfs_merged, sort_type = FALSE) +
  theme(
...
We call "visit CRFs" any CRFs that was not present in the "participants CRFs" list. These CRFs were expected to contain potentially time-varying variables collected at multiple visits.

Before merging data from these CRFs, we check that each of these CRFs includes both the `study_day` and `visit_code` variables. Any CRF that does not meet this requirement is excluded from the visit CRF list.
We also remove CRF29 (Adverse Events) and CRF34 (Protocol Deviation) because they does not contain visit_code.

```{r}

visits_crf_name <-
  setdiff(
    names(crf_clean[!names(crf_clean) %in% c("crf1", "crf101", "crf2", "crf102")]),
    participants_crf_name
  ) |>
  setdiff("crf27_long")
```
...
crf_list_renamed <-
  ...
  imap(function(df, name) {
    ...
    names(df)[names(df) %in% double_cols] <- paste0(
      names(df)[names(df) %in% double_cols],
      ".",
      name
    )
    df
...
visits_crfs_merged <-
  ...
  relocate(uid, pid, visit_code, study_day)
```
...
visits_crfs_merged |>
  select(pid, visit_code, new_sex_partner)
```
...
save(
  participant_crfs_merged,
  file = str_c(dir, "01_participant_crfs_merged_", today() |> str_replace_all("-", ""), ".Rdata")
)
save(
  visits_crfs_merged,
  file = str_c(dir, "01_visits_crfs_merged_", today() |> str_replace_all("-", ""), ".Rdata")
)
```
