plot_data_for_pid <- function(selected_pid, mae_sub) {
  ...
  all_visits <- mae_sub$visit_code |>
    unique() |>
    sort() |>
    as_tibble() |>
    dplyr::rename(visit_code = value)
  ...
  mg_ <-
    ...
    dplyr::left_join(
      coldata |> select(uid, pid, visit_code),
      by = join_by(uid)
    ) |>
  ...
  ampl_ <-
    ...
    mutate(.feature = .feature |> fct_inorder()) |>
    dplyr::filter(
      str_detect(.feature, "crispatus") | as.numeric(.feature) <= 10
    ) |>
    dplyr::left_join(
      coldata |> select(uid, pid, visit_code),
      by = join_by(uid)
  ...
  g_PCR <-
    qPCR_ |>
    mutate(
      copies_per_swab_med = ifelse(
        copies_per_swab_med == 0,
        NA,
        copies_per_swab_med
      )
    ) |>
    ...
    facet_grid(
      ifelse(
        is.na(.feature) | (.feature %in% c("16S", "LBP all")),
        "Total",
        "LBP strains"
      ) |>
