...
crf_clean_file <- crf_files |>
  str_subset("01_crf_clean") |>
  sort(decreasing = TRUE) |>
  magrittr::extract(1)
crf_raw_file <- crf_files |>
  str_subset("01_crf_raw") |>
  sort(decreasing = TRUE) |>
  magrittr::extract(1)
crf_dict_file <- crf_files |>
  str_subset("01_crf_plates_dictionary") |>
  sort(decreasing = TRUE) |>
  magrittr::extract(1)
...
load(
  crf_files |>
    str_subset(str_c("01_participants_", today() |> year())) |>
    sort(decreasing = TRUE) |>
    magrittr::extract(1),
  verbose = TRUE
)
load(
  crf_files |>
    str_subset("01_participants_variable_dictionary") |>
    sort(decreasing = TRUE) |>
    magrittr::extract(1),
  verbose = TRUE
)
load(
  crf_files |>
    str_subset("01_participant_crfs_merged") |>
    sort(decreasing = TRUE) |>
    magrittr::extract(1),
  verbose = TRUE
)
...
load(
  crf_files |>
    str_subset(str_c("01_visits_", today() |> year())) |>
    sort(decreasing = TRUE) |>
    magrittr::extract(1),
  verbose = TRUE
)
...
load(
  crf_files |>
    str_subset(str_c("01_visits_long_", today() |> year())) |>
    sort(decreasing = TRUE) |>
    magrittr::extract(1),
  verbose = TRUE
)
...
load(
  crf_files |>
    str_subset("01_visits_crfs_merged") |>
    sort(decreasing = TRUE) |>
    magrittr::extract(1),
  verbose = TRUE
)
...
cat(
  "Loading qPCR data for daily swabs from file: ",
  daily_qPCR_file |> basename(),
  "\n"
)
...
cat(
  "Loading 16S rRNA amplicon sequencing ASV-level data from file: ",
  amplicon_ASV_file |> basename(),
  "\n"
)
...
cat(
  "Loading 16S rRNA amplicon sequencing data aggregated by taxonomic assignment from file: ",
  amplicon_file |> basename(),
  "\n"
)
...
for (i in 1:nrow(assay_list)) {
  se <- get(assay_list$se_object_name[i])
  if (
    ("sample_type" %in% colnames(colData(se))) |
      ("control_type" %in% colnames(colData(se)))
  ) {
    colData(se) <- colData(se)[,
      !colnames(colData(se)) %in% c("sample_type", "control_type")
    ]
  }
...
mae_coldata <-
  ...
  dplyr::full_join(
    visits |>
      select(-starts_with("specimen_collection")) |>
      mutate(in_CRF = TRUE),
    mae_coldata_assays |>
      mutate(in_assays = TRUE) |>
      select(in_assays, everything()),
    by = join_by(uid)
  ) |>
  mutate(
...
mae_coldata <-
  mae_coldata |>
  mutate(
    needs_fixing = is.na(pid) &
      (sample_type == "Clinical sample") &
      str_detect(uid, "[0-9]{9}_[0-9]{4}"),
    pid = case_when(
    ...
    visit_attended = case_when(
      is.na(visit_attended) & needs_fixing ~ TRUE,
      TRUE ~ visit_attended
    ),
    visit_planned = case_when(
...
pid_with_missing_daily_study_days <-
  ...
  mutate(condition = all(is.na(diff))) |>
  ungroup() |>
  mutate(diff = ifelse(condition, 0, diff)) |> # if still missing,we set to 0
  # we fix the study_day
  mutate(
    fixed_study_day = case_when(
...
visit_dict |>
  ...
  cols_align(
    align = "left",
    columns = c(visit, definition, possible_visit_codes)
  )
```
...
mae_coldata |>
  ...
  dplyr::filter(
    randomized,
    (screening_visit == "main screening"),
    study_day == 0
  ) |>
  select(pid) |>
  distinct() |>
  ...
  bind_rows(
    ...
    tibble(
      ...
      description = case_when(
        variable ==
          "uid" ~ "Unique identifier for each participant x visit combination, in the format `pid`_`visit_code`",
        ...
        variable ==
          "study_week" ~ "Study week number (from -2 to 12, with 0 being the day of the pre-MTZ visit)",
        variable ==
          "study_day" ~ "Relative study day number with 0 being the day of the pre-MTZ visit",
        variable ==
          "n_distinct_study_days" ~ "Number of distinct study days for a given participant and visit number",
        variable ==
          "crf_plates" ~ "CRF plates for which we have data for this participant x visit combination",
        variable ==
          "visit_attended" ~ "Whether the participant attended the visit according to the CRF data",
        ...
        variable ==
          "in_assays" ~ "Whether this participant x visit combination is in an assay data",
        variable ==
          "sample_type" ~ "Sample type for this participant x visit combination in the assay data (Clinical sample, Clinical sample (other study), Negative/Positive control), Test sample or NA)",
        variable ==
          "control_type" ~ "Control type for the Negative and Positive control samples in the assay data (Nuclease-free water, Unused swab + C2, Mock 1 and Mock 2)",
        variable ==
          "assays" ~ "Names of the assays for which we have data for this participant x visit combination (mg, qPCR, amplicon, luminex, flow, etc.)",
        variable ==
          "screening_visit" ~ "Whether the visit is the main screening visit, an additional screening visit, or not a screening visit"
      )
    )
  )
```
...
se_sample_and_visit_info_assay <-
  SummarizedExperiment(
    assays = list(
      sample_and_visit_info = sample_and_visit_info_assay |>
        select(-c(sample_type, in_CRF, in_assays, randomized)) |>
        as.data.frame() |>
        column_to_rownames("uid") |>
        t()
    ),
    colData = sample_and_visit_info_assay |>
      select(uid) |>
      mutate(rownames = uid) |>
      as.data.frame() |>
...
sample_manifest_summary_by_uid <-
  ...
  summarize(
    ...
    ext_batches = str_c(
      unique(ext_lib_plate_batch) |> str_replace_na("NA"),
      collapse = " | "
    ),
    n_plate_nb = n_distinct(ext_lib_plate_nb),
    plate_nbs = str_c(
      unique(ext_lib_plate_nb) |> str_replace_na("NA"),
      collapse = " | "
    ),
...
sample_manifest <-
  ...
  mutate(
    ...
    ext_lib_plate_nb_num = ext_lib_plate_nb |>
      str_remove(",.*") |>
      parse_number(),
    ext_lib_info_source = case_when(
      !is.na(ext_lib_plate_nb) &
        (ext_lib_plate_nb == ext_lib_plate_nb_origin) ~ "colData of source assay",
...
sample_manifest |>
  filter(is.na(ext_lib_plate_id), sample_type == "Clinical sample")
```
...
sample_manifest |>
  filter(
    ext_lib_plate_batch == "Daily swab",
    sample_type == "Clinical sample"
  ) |>
...
sample_manifest <-
  ...
  mutate(
    ...
    ext_lib_plate_id = case_when(
      uid == "068200278_1404" ~ "Daily swabs plate 21" |>
        factor(levels = levels(ext_lib_plate_id)),
      uid == "068200356_1305" ~ "Daily swabs plate 19" |>
        factor(levels = levels(ext_lib_plate_id)),
      uid == "068200385_1205" ~ "Daily swabs plate 17" |>
        factor(levels = levels(ext_lib_plate_id)),
      uid == "068200385_1206" ~ "Daily swabs plate 17" |>
        factor(levels = levels(ext_lib_plate_id)),
    ...
    ext_lib_info_source = case_when(
      uid %in%
        c(
          ...
          "068200385_1205",
          "068200385_1206"
        ) ~ "imputed from surrounding samples of the same participant",
  ...
  MultiAssayExperiment::MultiAssayExperiment(
    ...
    metadata = list(
      ...
      sample_manifest = sample_manifest |>
        select(uid, barcode, source_assay = source, starts_with("ext_lib_"))
    )
  )
```
...
mae_sample_map <-
  mae@sampleMap |>
  as_tibble() |>
  dplyr::filter(assay %in% assay_list$mae_assay_name) |>
  select(uid = primary, assay) |>
  arrange(uid) |>
  dplyr::left_join(
    assay_list |> dplyr::rename(assay = mae_assay_name),
    by = join_by(assay)
  ) |>
  dplyr::right_join(
    mae@colData |>
