  ...
  mutate(
    ext_lib_plate_id = 
      str_c(
        ...
        str_pad(min_plate_nb, width = 2, pad = "0")
        ),
    ext_lib_plate_nb = min_plate_nb
  ) |> 
  arrange(ext_lib_plate_batch, ext_lib_plate_nb) |> 
  mutate(ext_lib_plate_id = ext_lib_plate_id |> fct_inorder()) |> 
  select(uid, ext_lib_plate_batch, ext_lib_plate_nb, ext_lib_plate_id)
...
sample_manifest <- 
  sample_manifest |> 
  select(uid, barcode, source, sample_type, visit_type, ext_lib_plate_batch, ext_lib_plate_nb) |> 
  left_join(
    ext_plate_amplicon |> 
      select(
        ext_lib_plate_batch_new = ext_lib_plate_batch, 
        ext_lib_plate_nb_new = ext_lib_plate_nb,
        ext_lib_plate_id_new = ext_lib_plate_id
        ) |> 
      mutate(source = "16S_ASV")
  )
...
:::callout
Once this is done, update the code for the MAE creation to keep the extraction library plate numbers.
:::
...
mae <- 
  MultiAssayExperiment::MultiAssayExperiment(
    ...
    metadata = list(
      ...
      exposures = exposures,
      sample_manifest = sample_manifest |> select(uid, barcode, source_assay = source)
    )
  )
