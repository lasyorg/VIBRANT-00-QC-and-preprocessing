...
sample_manifest |> 
  dplyr::count(ext_lib_plate_nb, ext_lib_plate_id, ext_lib_plate_type, pcr_plate_id, pcr_plate_nb)


```


We now check that all samples with the same `uid` (and `barcode`) have consistent extraction data across assays.
