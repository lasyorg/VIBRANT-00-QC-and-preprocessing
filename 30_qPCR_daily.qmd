...
We noticed a typo in `ext_lib_position`., some sample are labbeled "_h9" and some "_H9". We fix this issue. 

```{r}

# TODO : fix the ext_lib_position: replace h9 by H9 (carefully?!)
...
There are two samples that have replicates on different extraction plates (`ext_lib_plate_nb` or `ext_lib_plate_id`). 
...
tmp |> 
  ...
  gt("Sample IDs replicated in the extraction plates")

```

We change the `vibr_sample_id` of the 2 samples that have 6 replicates in the gDNA (extraction) by adding "_1" and "_2" depending on their gDNA position
...
This is unexpected because we thought that each sample was only extracted once. 
...
We harmonized standard names between 16S and LBP : 
...
qpcr <-  
  ...
  mutate(
    content = 
      case_when(
      str_detect(content, regex("^\\s*Std\\s*-\\s*0?[1-7]\\s*$", ignore_case = TRUE)) ~ "Std",
      #str_detect(sample,  regex("^\\s*standard\\s*[1-6]\\s*$",    ignore_case = TRUE)) ~ "Standard",
      TRUE ~ content
    ), 
    sample =  
      case_when(
    ...
    qpcr_sample_type = 
      case_when(
        ...
        str_detect(content, "Unkn") & is.na(replicate) & is.na(vibr_sample_id) ~ "Likely empty",
...
tmp |> dplyr::count(n_plates) # *Almost* all samples's replicates are on a single plate
# TODO: check what are the samples that are on several plates

tmp |> dplyr::count(n_replicates, n_plates) # All samples have at least 3 replicates
...
rep_per_plate <-
  ...
  mutate(well = str_c(well_row, well_col)) |>
  distinct(vibr_sample_id, target, pcr_plate_id, well) |>
  dplyr::count(vibr_sample_id, target, pcr_plate_id, name = "n_wells_plate")


rep_per_plate |>
  group_by(vibr_sample_id, target) |>
  summarise(
    n_reps_total = sum(n_wells_plate),
    n_qpcr_plates = n_distinct(pcr_plate_id),
    breakdown = str_c(paste0(pcr_plate_id, "=", n_wells_plate), collapse = "; "),
    .groups = "drop"
  ) |> 
  filter(n_reps_total == 6) |> 
  arrange(vibr_sample_id, target)
