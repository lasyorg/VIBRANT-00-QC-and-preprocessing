...
A subset of wells has no vibr_sample_id, yet each of these wells has a valid gDNA extraction coordinate (ext_lib_plate_id/ext_lib_position) and a measurable Cq. Within the same qPCR run and plate layout, the corresponding wells carry valid identifiers for earlier VMRC targets but become blank for later VMRC targets.

We think this pattern is consistent with a metadata transcription/propagation error during target-specific exports: the plate map (sample IDs) was entered or propagated for the first VMRC assays and not copied forward for the subsequent assays. In other words, the physical samples were present and measured, but the identifier field was left empty for some targets. This is a documentation issue rather than a biological or assay failure.

We will correct this by replacing the missing "vibr_sample_id" by plate according to the position on the previous VRMC plate. 
...
qpcr |> 
  ...
  select(pcr_plate_id, ext_lib_plate_nb, strain_group_qpcr, target, well_col, well_row, sample, vibr_sample_id) |>
  distinct() |>
  ggplot() +
  aes(x =  well_row |> factor(), y = well_col |> factor() |> fct_rev(), fill = !is.na(vibr_sample_id)) +
  facet_grid(pcr_plate_id ~ strain_group_qpcr + target) +
  geom_tile() +
  geom_path(aes(group = sample), col = "black", alpha = 0.5) +
  geom_point() +
  scale_fill_manual("vibrant sample id", values = c("steelblue", "lightgreen")) +
