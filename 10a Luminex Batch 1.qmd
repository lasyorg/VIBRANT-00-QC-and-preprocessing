...
plate_dirs <- list.files(batch1_dir, full.names = TRUE, pattern = "late [0-9]")
...
manifest <- 
  read_xlsx(
    str_c(batch1_dir, "/Plate Layouts.xlsx"), 
...
```
::: callout-caution
@Lenine: it looks like the weight values for the US have been rounded in the manifest, but when you computed them, they were likely not rounded. 
Would you be able to resend a file with the non-rounded values?
:::
...
```
::: callout-note
The figure above shows that the fluorescence intensity of S9 and S10 are overlapping with the fluorescence intensity of the blanks wells. This suggests that S9 and S10 might not be necessary for batch 2.
:::
...
It looks like for a few analytes and plates (e.g, IL-7 plate 1, or IL-5 plate 2), the calibration curves were not fitted properly. It might improve data quality to systematically identify poor calibration curve fits and re-fit them. 
...
> TODO once we have all the data
