...
crf14 |> 
  ...
  select(pid, visit_code, nugent_total_score, nugent_lactobacillus, nugent_gardenella, nugent_mobiluncus) |>
  gt()

crf14 <- 
  crf14 |> 
  mutate(
    nugent_flag_inconsistent = (nugent_total_score !=  nugent_lactobacillus + nugent_gardenella + nugent_mobiluncus)
  )
...
exposures <- 
  ...
  mutate(daily_self_reported_adherence_sp = case_when(
...
exposures |>
  ...
  ggplot(aes(x = factor(visit_code), y = factor(pid), color = daily_self_reported_exposure_to_mtz)) +
  geom_point() +
...
exposures |>
  mutate(site = ifelse(str_detect(pid, "0681"), "MGH", "CAP")) |>
  ggplot(aes(x = factor(visit_code), y = factor(pid), color = daily_self_reported_adherence_sp |> as.factor())) +
  geom_point() +
  facet_grid(site ~ ., scales = "free_y", space = "free") +
  scale_color_discrete(na.translate = FALSE) +  
  scale_color_manual(
