...
:::callout-note
The purpose of this document is to clean, format, check, and correct/harmonized when needed the data collected via the VIBRANT CRFs.

From the "raw" data provided as an `R list` by the CAPRISA data team, we create two output lists:

- `crf_raw`, a `list` very similar to the raw data and which contains one element (a `tibble`) per CRF but from which "sensitive information" (such as visit dates, comments, etc.) and metadata pertaining to the quality checks performed by the CAPRISA data team have been removed.

- `crf_clean` which also contains one element per CRF, but where the columns have been formatted, transformed, cleaned, and prepared for downstream analyses.
:::

The document is organized as follow: after loading the raw data, we perform a few automatic cleaning actions on all CRF tables, then, go through each CRF one by one to perform cleaning actions specific to that CRF.

# Raw data

We load the raw CRF data shared by Lara Lewis on August 7th 2025.
...
In this section, we perform harmonization that apply to all CRFs.
...
- transform the `vdate` from a `character` variable into a `date`,
- define a `study_day` variable that provides a relative date from visit V1 (Enrollment visit, day 0) - to do so, we define a table with the V1 date for each participant (see below);
- define a `flag_date` variable that flags visits that may have mistakes in their visit dates (*e.g*, they occurred before the study started or after it ended, or for a given participant, outside their enrollment window).
...
study_start_date <- as.Date("2023-10-19") # The official study start was 2024-02-01 https://clinicaltrials.gov/study/NCT06135974?term=NCT06135974&rank=1 but a few participants were enrolled prior to that date.
...
day_0_dates |>
  ...
  geom_vline(
    xintercept = c(study_start_date, study_end_date),
    linetype = "dashed",
    color = "black"
  ) +
  geom_segment(
    aes(x = day_0_date, xend = expected_end_date, yend = pid |> factor()),
    alpha = 0.25
  ) +
  geom_point() +
  ...
  if ("vdate" %in% colnames(crf_data)) {
    crf_data <-
      ...
      mutate(
        ...
        flag_date = (vdate < study_start_date) |
    ...
    if (any(crf_data$flag_date, na.rm = TRUE)) {
      cat(
        ...
        crf_name,
        "contains",
        sum(crf_data$flag_date, na.rm = TRUE),
        "visits before the study start or end date or after the participant was expected to complete the study.\n"
      )
    }
  }
  return(crf_data)
...
all_visit_dates <-
  ...
  imap(
    ~ .x |>
      select(any_of(c("pid", "dfseq", "vdate", "vdate_original", "flag_date"))) |>
      mutate(crf = .y)
  ) |>
  bind_rows() |>
  arrange(pid, dfseq) |>
...
all_visit_dates |>
  ...
  mutate(
    pid = pid |> factor(),
    random_selection = as.numeric(pid) %in% (sample(1:22, 1) + c(1:5))
  ) |> #
...
all_visit_dates |>
  filter(pid == "068200407") |>
  ggplot() +
  aes(x = vdate_fixed, y = crf_nb |> factor() |> fct_rev(), col = new_flag, label = crf_nb) +
  geom_text(alpha = 0.7, size = 3) +
  facet_grid(dfseq |> factor() ~ ., scales = "free", space = "free") +
...
- we do not fix the dates from CRFs with no visit code (`dfseq`)
...
# 068200150 date for visit code 1302 is likely wrong (by 1 day), it should be JUN 2 and not JUN 3
# 068100061 visit code 1000 to 1005 in crf 32 are likely wrong by 1 number - they should be 1001 to 1006
...
all_visit_dates <-
  ...
  mutate(
    vdate_fixed = case_when(
      ...
      (pid == "068200150") & (crf_nb %in% c(32)) & (dfseq == 1302) ~ as.Date("2024-06-02"),
      ...
      new_flag & (pid == "068100061") & (crf_nb == 32) & (dfseq %in% c(1000:1005)) ~ vdate_fixed, # this is likely correct, but we fix the visit code below
    ...
    dfseq = case_when(
      ...
      (pid == "068100061") & (crf_nb == 32) & (dfseq %in% c(1000:1005)) ~ dfseq + 1, # we fix the visit code for crf 32
...
raw_data_list$cap068_data_32 <-
  ...
  mutate(
    dfseq = case_when(
      (pid == "068100061") & (dfseq %in% c(1000:1005)) ~ dfseq + 1, # we fix the visit code for crf 32
      TRUE ~ dfseq
    )
  )
...
Since some dates were fixed, we recompute the `day_0_dates` table as we'll use it downstream.
...
all_visit_dates <-
  ...
  imap(
    ~ .x |>
      select(any_of(c("pid", "dfseq", "vdate", "vdate_original", "study_day"))) |>
      mutate(crf = .y)
  ) |>
  bind_rows() |>
  arrange(pid, dfseq) |>
...
We check that computed relative study days are consistent with our expectations given the study design and protocol.
...
all_visit_dates |>
  ...
  aes(
    ...
    y = pid |> factor(),
    shape = is_daily_code,
    col = study_day == expected_study_day,
    label = crf_nb
  ) +
...
add_VIBRANT_study_code_and_uid <- function(crf_data, crf_name) {
  if (!("dfseq" %in% colnames(crf_data))) {
    ...
    if ("visit_code" %in% colnames(crf_data)) {
      warning("The CRF already contains a visit_code column, it will be overwritten!")
    }
    if ("uid" %in% colnames(crf_data)) {
      warning("The CRF already contains a uid column, it will be overwritten!")
    }
    crf_data <-
      crf_data |>
      mutate(
...
plot_marginal_dist <- function(crf) {
  ...
  for (var in colnames(crf)) {
    ...
    if (
      !(var %in% c("pid", "dfseq", "vdate", "vdate_fixed", "datecompleted", "study_day", "uid"))
    ) {
  ...
  for (j in 1:ncol(crf)) {
    if (
      ...
      if (length(unique(crf[, j] |> unlist())) == 1) {
        ...
        cat(
          "Only one unique value in the variable",
          colnames(crf)[j],
          " (= ",
          unique(crf[j] |> unlist()),
...
crf1 <-
  ...
  mutate(
    ...
    ethn = str_c(ifelse(hispanic_latino_spanish == "No", "Not ", ""), "hispanic/latino/spanish") |>
      fct_infreq(),
...
additional_income_sub <-
  ...
  pivot_longer(
    cols = -c(pid, dfseq),
    names_to = "additional_income_type",
    values_to = "income",
    names_prefix = "additional_income_"
  ) |>
  mutate(income = (tolower(income) == "checked")) |>
  group_by(pid, dfseq) |>
  summarize(
    additional_income_type = str_c(additional_income_type[income], collapse = ", "),
    .groups = "drop"
  ) |>
  mutate(
    additional_income_type = case_when(
      additional_income_type == "" ~ "No additional income",
...
crf3 |>
  left_join(
    crf_raw$crf3 |>
      select(
        uid,
        other_test_type1_specify,
        other_test_type2_specify,
        hiv_results_not_required,
        hiv_rapid_test1,
        test_type1,
        hiv_rapid_test2,
        test_type2,
        mgh_abbott_hiv_results_not_requi,
        mgh_abbott_hiv_test_result
      )
  )
```
...
bind_rows(
  crf6 |>
    pivot_longer(
      cols = -c(uid, pid, visit_code, dfseq, vdate_fixed, study_day),
      names_to = "variable",
      values_to = "value"
    ) |>
    mutate(crf = "CRF6"),
  crf7 |>
    pivot_longer(
      cols = -c(uid, pid, visit_code, dfseq),
      names_to = "variable",
      values_to = "value"
    ) |>
    mutate(crf = "CRF7")
...
crf8 <-
  ...
  mutate(
    ...
    start_hormone_contraception_date = case_when(
      !(month_start_hormone_contraceptio %in%
        c(
          "JAN",
          "FEB",
          "MAR",
      ...
      TRUE ~ str_c(
        "01",
        month_start_hormone_contraceptio,
        "20",
        year_start_hormone_contraception |> str_pad(width = 2, pad = "0")
      ) |>
    ...
    other_protection_specify = ifelse(
      is.na(other_protection_specify),
      "unckecked",
      other_protection_specify
    ),
  ) |>
  select(
    -c(
      month_start_hormone_contraceptio,
      year_start_hormone_contraception,
...
protection_sub <-
  ...
  group_by(pid, dfseq) |>
  summarize(
    protection_type = str_c(protection_type[protection] |> sort(), collapse = ", "),
    .groups = "drop"
  ) |>
...
for (c in 1:dim(pregnancy_sub)[1]) {
  if (pregnancy_sub$ever_been_pregnant[c] == "no") {
    if (
      pregnancy_sub$number_pregnancies[c] != 0 ||
        pregnancy_sub$unplanned_pregnancies[c] != 0 ||
        pregnancy_sub$number_deliveries[c] != 0
    ) {
      cat(
        "Error: Pid ",
        pregnancy_sub$pid[c],
        " has a wrong number of pregnancy, she's never been pregnant but her number of pregnancy or delivery is not 0!",
        "\n"
      )
    }
  }
...
crf8 <-
  ...
  select(
    ...
    dfseq,
    study_day,
    begin_hormone_contraception,
    start_hormone_contraception,
    date_start_menses
  ) |>
...
antibiotic_sub <-
  ...
  select(
    ...
    do_not_know,
    none = antibiotic_none
  ) |>
  pivot_longer(
    cols = -c(pid, uid, dfseq),
    names_to = "antibiotic_type",
    values_to = "antibiotic"
  ) |>
  mutate(antibiotic = (tolower(antibiotic) == "checked")) |>
  group_by(pid, uid, dfseq) |>
  summarize(
    antibiotic_type = str_c(antibiotic_type[antibiotic], collapse = ", "),
    .groups = "drop"
  ) |>
...
relationship_sub <-
  ...
  group_by(pid, uid, dfseq) |>
  summarize(
    relationship_type = str_c(relationship_type[relationship], collapse = ", "),
    .groups = "drop"
  ) |>
...
partners_sub <-
  ...
  select(
    pid,
    dfseq,
    sexual_partners_lifetime,
    sexual_partners_past_month,
    past_relation_harmed,
    current_relation_harmed
  )
...
crf14 <-
  ...
  mutate(
    ...
    amsel_criteria = (clue_cells == "Positive") +
      (whiff_test == "Positive") +
...
The Nugent score is the sum of the score for lactobacillus, gardenella and mobiluncus, so we check that the `nugent_total_score` variable is consistent with the sum of the three other variables. Four entries are not consistent, we flag them in `nugent_flag_inconsistent`.

```{r}

crf14 |>
  filter(nugent_total_score != nugent_lactobacillus + nugent_gardenella + nugent_mobiluncus) |>
  select(
    pid,
    visit_code,
    nugent_total_score,
...
crf14 <-
  crf14 |>
  mutate(
    nugent_flag_inconsistent = (nugent_total_score !=
      nugent_lactobacillus + nugent_gardenella + nugent_mobiluncus)
  )
```
...
crf15 <-
  ...
  # filter(!is.na(date_completed)) |> #datecompleted
  mutate(
    pap_smear_results = ifelse(
      tolower(pap_smear_not_applicable) == "checked",
      NA,
      pap_smear_results
    ),
...
crf16 <-
  ...
  mutate(
    ...
    compare_week_before = compare_week_before |>
      factor(
        levels = c(
...
crf17 <-
  ...
  # filter(!is.na(date_completed)) |> #datecompleted
  mutate(
    vaginal_discharge = ifelse(
      tolower(vaginal_discharge) == "yes",
      how_much_discharge,
      vaginal_discharge
    ),
    ...
    compare_pain_week_before = compare_pain_week_before |>
      factor(
        levels = c(
          "Better than the week before",
          "The same as week before",
    ...
    compare_discharge = compare_discharge |>
      factor(
        levels = c(
          "Better than the week before",
          "The same as week before",
...
crf18 <-
  ...
  mutate(
    ...
    compare_dryness = compare_dryness |>
      factor(
        levels = c(
          "Better than the week before",
          "The same as week before",
...
crf19 <-
  ...
  mutate(
    ...
    symptom_interfere_sex = ifelse(
      tolower(ability_have_sex) == "yes",
      symptoms_often_interfere,
      ability_have_sex
    ),
    symptom_interfere_sex = symptom_interfere_sex |>
      fct_infreq() |>
      fct_relevel("Rarely", after = 2L),
    vaginal_bleeding = ifelse(vaginal_bleeding %in% c("Blank", "Not done"), NA, vaginal_bleeding) |>
      fct_infreq(), #|> fct_relevel("Blank", after = Inf)
    past_week_sex_partner = past_week_sex_partner |> fct_infreq(),
    new_sex_partner = case_when(
    ...
    use_condoms = ifelse(use_condoms %in% c("Blank", "Not done"), NA, use_condoms) |>
      factor(
...
past_week_sub <-
  ...
  group_by(pid, dfseq) |>
  summarize(
    past_week_type = str_c(past_week_type[past_week] |> sort(), collapse = ", "),
    .groups = "drop"
  ) |>
...
sex_past_24hours_sub <-
  ...
  pivot_longer(
    cols = -c(pid, dfseq),
    names_to = "sex_past_24hours_type",
    values_to = "sex_past_24hours"
  ) |>
  mutate(sex_past_24hours = (tolower(sex_past_24hours) == "checked")) |>
  group_by(pid, dfseq) |>
  summarize(
    sex_past_24hours_type = str_c(
      sex_past_24hours_type[sex_past_24hours] |> sort(),
      collapse = ", "
    ),
    .groups = "drop"
  ) |>
...
clean_outside_sub <-
  ...
  pivot_longer(
    cols = -c(pid, dfseq),
    names_to = "clean_outside_type",
    values_to = "clean_outside"
  ) |>
  mutate(clean_outside = (tolower(clean_outside) == "checked")) |>
  group_by(pid, dfseq) |>
  summarize(
    clean_outside_type = str_c(clean_outside_type[clean_outside] |> sort(), collapse = ", "),
    .groups = "drop"
  ) |>
...
crf20 <-
  ...
  mutate(
    ...
    sex_past_24hours = ifelse(
      tolower(sex_past_24hours) == "yes",
      as.character(sex_past_24hours_type),
      sex_past_24hours
    ),
...
crf21 <-
  ...
  mutate(
    ...
    other_contraceptive_specify = ifelse(
      is.na(other_contraceptive_specify),
      "unchecked",
      other_contraceptive_specify
    )
  )
...
clean_vagina_inside_sub <-
  ...
  pivot_longer(
    cols = -c(pid, dfseq),
    names_to = "clean_vagina_inside_type",
    values_to = "clean_vagina_inside"
  ) |>
  mutate(clean_vagina_inside = (tolower(clean_vagina_inside) == "checked")) |>
  group_by(pid, dfseq) |>
  summarize(
    clean_vagina_inside_type = str_c(
      clean_vagina_inside_type[clean_vagina_inside] |> sort(),
      collapse = ", "
    ),
    .groups = "drop"
  ) |>
...
clean_vagina_often_sub <-
  ...
  select(
    pid,
    dfseq,
    clean_vagina_after_sex,
    clean_vagina_bathing,
    clean_vagina_before_sex,
    clean_vagina_never,
    clean_vagina_once_a_day,
    clean_vagina_other,
    often_clean_other
  ) |>
  pivot_longer(
    cols = -c(pid, dfseq),
    names_to = "clean_vagina_often",
    values_to = "vagina_often"
  ) |>
  mutate(vagina_often = (tolower(vagina_often) == "checked")) |>
  group_by(pid, dfseq) |>
  summarize(
    clean_vagina_often = str_c(clean_vagina_often[vagina_often] |> sort(), collapse = ", "),
    .groups = "drop"
  ) |>
...
clean_vagina_sub <-
  ...
  mutate(
    ...
    last_clean_vagina = ifelse(
      tolower(last_clean_other) == "unchecked",
      last_clean_vagina,
      last_clean_other |> str_to_sentence()
    )
  ) |>
...
put_in_vagina_sub <-
  ...
  pivot_longer(
    cols = -c(pid, dfseq),
    names_to = "put_in_vagina_type",
    values_to = "put_in_vagina"
  ) |>
  mutate(put_in_vagina = (tolower(put_in_vagina) == "checked")) |>
  group_by(pid, dfseq) |>
  summarize(
    put_in_vagina_type = str_c(put_in_vagina_type[put_in_vagina] |> sort(), collapse = ", "),
    .groups = "drop"
  ) |>
...
contraceptive_sub <-
  crf21 |>
  mutate(
    contraception = ifelse(
      tolower(other_contraceptive_specify) == "unchecked",
      contraception,
      other_contraceptive_specify
    )
  ) |>
...
crf21 <-
  crf21 |>
  mutate(
    contraceptive_missed_doses = ifelse(
      contraceptive_missed_doses %in% c("Missing", "Not done"),
      NA,
    ...
    contraception = case_when(
      ...
      str_detect(
        contraception,
        "Continuous combined oral contraceptives pills (skip placebo)"
      ) ~ "Continuous COC pills",
      str_detect(contraception, "Missing") |
        str_detect(contraception, "Blank") |
        str_detect(contraception, "N/A") ~ NA_character_,
      str_detect(
        contraception,
        "NO LONGER TAKING BC PILLS; 3 MONTHS SUPPLY COMPLETED"
...
product_missed_sub <-
  ...
  pivot_longer(
    cols = -c(pid, dfseq),
    names_to = "product_missed_day",
    values_to = "product_missed"
  ) |>
...
product_missed_sub <-
  product_missed_sub |>
  mutate(
    n_missed_days = case_when(
      (pid == "100010") & (dfseq == 1100) ~ 1,
      TRUE ~ n_missed_days
    ),
    product_missed_days = case_when(
...
crf23 |>
  ...
  gt::gt(
    caption = "Non-group4 participants for whom the number of remaining tablets and number of missed days does not match"
  )
```
...
For study product exposures, we create two variables, `daily_self_reported_adherence_sp` and `daily_self_reported_exposure_sp`, as well as a `comment_studyproduct` variable.


-   `daily_self_reported_adherence_sp`: a binary flag indicating whether the participant reported **inserting** the study product on that day. This variable is constructed using data from CRF23 (weekly study product form) and CRF32 (daily diary). We first use the `insert_study_product_time` variable from CRF32 to assign value of 1 when participants reported an insertion time (i.e. another value than "Not applicable" or "None"). We then use the CRF23 to adjust the previous number by reading the comments (e.g. some participants fill the `insert_study_product_time` but comments from CRF32 precise that participants does not take the tablets that day) 


-   `daily_self_reported_exposure_sp`: a continuous variable ranging from 0 to 1, indicating whether the study product **remained** in the vagina after insertion. Many participants reported that the (fully intact) tablet fell out (e.g., into the toilet bowl) a few hours after insertion.  
...
exposures <-
  exposures |>
  mutate(
    daily_self_reported_exposure_to_mtz = case_when(
...
exposures <-
  ...
  mutate(
    comments_mtz = case_when(
      ...
      pid ==
        "068200253" ~ "4 evening doses forgot but we don't knwo when (we put 4 days with 1 doses)",
    ...
    daily_self_reported_exposure_to_mtz = case_when(
      ...
      TRUE ~ as.character(daily_self_reported_exposure_to_mtz)
    )
  )
```
...
exposures_studyproduct <-
  ...
  mutate(daily_self_reported_adherence_sp = 1)
...
exposures |>
  group_by(pid) |>
  mutate(
    n_study_product_doses = sum(daily_self_reported_adherence_sp, na.rm = TRUE),
  ) |>
  filter(
    n_study_product_doses != 7,
    daily_self_reported_adherence_sp == 1
  )
...
tmp <-
  ...
  slice_max(study_day) |>
  mutate(
    n_study_doses_product_crf23 = across(day1:day7, ~ .x == "Unchecked") |>
      rowSums()
  ) |>
...
inconsistency <-
  ...
  group_by(pid) |>
  mutate(
    n_study_product_doses_crf32 = sum(daily_self_reported_adherence_sp, na.rm = TRUE),
  ) |>
...
exposures <-
  ...
  mutate(
    comments_studyproduct = case_when(
      pid ==
        "068100036" ~ "3 tablets fell AFTER insertion; no indication of when during treatment.",
      ...
      pid ==
        "068100053" ~ "In CRF32, four insertion times are reported, but CRF23 shows only one dose taken and six returned. The participant stopped because she felt sick.",
      ...
      pid ==
        "068200020" ~ "1 tablet dropped BEFORE insertion (not taken, 6 doses total).\n  Last dose missing in CRF32 and CRF23, but CRF32 comment seems inconsistent.",
      pid == "068200225" ~ "1 tablet fell out AFTER insertion (last dose, visit code 1107)",
      pid ==
        "068200028" ~ "3 tablets fell out AFTER insertion; no indication of when during treatment.",
      pid ==
        "068200347" ~ "Only 6 doses in crf32 but no missing doses in crf23 --> add a doses 11001",
      TRUE ~ NA_character_
    )
  ) |>
  mutate(
    daily_self_reported_adherence_sp = case_when(
      ...
      TRUE ~ daily_self_reported_adherence_sp
    ),
    daily_self_reported_exposure_sp = case_when(
...
exposures |>
  ...
  ggplot(aes(
    x = factor(visit_code),
    y = factor(pid),
    color = daily_self_reported_exposure_to_mtz
  )) +
  geom_point() +
  ...
  labs(title = "Exposures metronidazole", x = "Visit code", y = "Pid")
```
...
exposures |>
  ...
  ggplot(aes(
    x = factor(visit_code),
    y = factor(pid),
    color = daily_self_reported_adherence_sp |> as.factor()
  )) +
  geom_point() +
  facet_grid(site ~ ., scales = "free_y", space = "free") +
  scale_color_discrete(na.translate = FALSE) +
  ...
  labs(title = "Exposures study product (insertion)", x = "Visit code", y = "Pid")

exposures |>
  mutate(site = ifelse(str_detect(pid, "0681"), "MGH", "CAP")) |>
  ggplot(aes(x = factor(visit_code), y = factor(pid), color = daily_self_reported_exposure_sp)) +
  geom_point() +
  facet_grid(site ~ ., scales = "free_y", space = "free") +
  ...
  labs(title = "Exposures study product", x = "Visit code", y = "Pid")
```
  ...
  append(
    list(
      crf25 = crf25 |>
        select(
          -c(
            any_of(columns_to_remove_for_raw),
            reason_longer_shorter,
            reason_use_product_again,
            reason_use_applicator_again,
            reason_benefit_you,
          )
        ) |>
        select(
...
crf25 <-
  ...
  mutate(
    product_shorter_longer_how_many = case_when(
      product_shorter_longer == "Shorter" ~ paste0(
        product_shorter_longer,
        " (",
        shorter_days,
        " d)"
      ),
...
crf27_long <-
  ...
  select(
    -c(
      sample_visit_date,
      vdate_fixed,
      alt_neisseria_gonorrhoea,
      alt_chlamydia_trachomatis,
      alt_trichomonas_vaginalis,
      alt_syphilis,
      alt_hsv1_hsv2,
      alt_buds_hyphae
    )
  ...
  mutate(
    ...
    test_results = case_when(
      test_results_original == "Positive" |
        test_results_original == "Detected" |
        test_results_original == "Present" ~ "Positive",
      test_results_original == "Negative" |
        test_results_original == "Not detected" |
        test_results_original == "Absent" ~ "Negative",
...
crf29_bis <-
  ...
  summarise(
    ...
    n_adverse_events = n(),
    highest_severity = if_else(
      any(severity == "Grade 2- Moderate"),
      "Grade 2- Moderate",
      "Grade 1- Mild"
    ),
...
crf30 <-
  ...
  mutate(
    vulvar = pmap_chr(
      ...
      ~ c("erythema", "edema", "rash", "tenderness", "bartholins_skene_gland")[
        c(...) == "Checked"
      ] |>
        str_c(collapse = "&")
    ),
    ...
    vaginal = pmap_chr(
      ...
      ~ c("erythema", "edema", "masses", "abrasions", "tenderness")[c(...) == "Checked"] |>
        str_c(collapse = "&")
    ),
    ...
    cervical = pmap_chr(
      ...
      ~ c("erythema", "edema", "masses", "motion_tenderness", "discharge")[c(...) == "Checked"] |>
        str_c(collapse = "&")
    ),
...
```{r}
crf31 <- crf31 |>
  select(-any_of(columns_to_remove_for_clean), -staff_initial_start, -staff_init_stop)
```
...
time_of_day_levels <-
  c(crf32$insert_study_product_time, crf32$vaginal_swab_collection_time) |>
  unique() |>
  sort() |>
  factor(
    levels = c(
...
crf32 <-
  crf32 |>
  mutate(
    insert_study_product_time = insert_study_product_time |>
      factor(levels = time_of_day_levels),
    vaginal_swab_collection_time = vaginal_swab_collection_time |>
      factor(levels = time_of_day_levels)
  )
```
...
crf32 |>
  ...
  geom_point() +
  facet_grid(
    ifelse(group4 == "Yes", "Group 4", "Other groups") + site ~ .,
    scales = "free",
    space = "free"
  ) +
  theme(
...
crf32 |>
  ...
  scale_color_manual(name = "Same time?", values = c("darkblue", "red")) +
...
sex_practices_sub <-
  ...
  pivot_longer(
    cols = -c(pid, dfseq, sex_past_24hours),
    names_to = "sex_type",
    values_to = "value"
  ) |>
  mutate(
...
put_in_vagina_33_sub <-
  ...
  mutate(
    ...
    put_in_vagina_type = put_in_vagina_type |>
...
crf33 <-
  ...
  mutate(
    condom_use = condom_use |>
      factor(levels = c("Never", "Sometimes", "Always", "Unknown", "Blank")),
    lubricant_use = lubricant_use |>
      factor(levels = c("Never", "Sometimes", "Always", "Unknown", "Blank")),
...
crf34 <-
  ...
  left_join(day_0_dates |> select(pid, day_0_date), by = "pid") |>
  mutate(
    relative_date_violation_deviation = as.numeric(
      as.Date(date_violation_deviation, format = "%d%b%y") - day_0_date
    )
  ) |>
  mutate(
...
crf35 <-
  crf35 |>
  mutate(
    specimen1_specify = specimen1_specify |>
      str_replace("SPA", "PSA") |>
      str_remove("SWAB") |>
      str_remove("FOR") |>
      str_remove("^ ") |>
...
crf_sub <-
  ...
  select(
    ...
    follow_up_behaviour,
    bacterial_vaginosis,
    metronidazole_adherence,
    study_product_adherence,
    vital_signs,
    pelvic_exam,
    pap_smear,
    product_acceptability,
    daily_diary_sex_behaviour,
    food_frequency,
    reaps,
    protocol_deviation,
    adverse_event,
...
crf46 <-
  crf46 |>
  mutate(
    total = applicator_stain_positive +
...
crf46 |>
  ...
  full_join(
    crf23 |>
      select(pid, used_applicators, tablets_remain) |>
      group_by(pid) |>
      summarize(
...
crf47_long <-
  ...
  pivot_longer(
    ...
    names_to = "home_swab_nb",
    values_to = "home_swab_visit_code"
  ) |>
  mutate(
...
save(
  crf_plates_dictionary,
  file = str_c(dir, "01_crf_plates_dictionary_", today() |> str_replace_all("-", ""), ".Rdata")
)
```
