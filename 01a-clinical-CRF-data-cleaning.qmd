...
crf14 |> 
  ...
  count(site, vaginal_ph_less_than, vaginal_ph_value) |> 
  gt()
...
exposures <- 
  ...
  left_join(
    raw_data_list$cap068_data_32 |> select(pid, visit_code, study_day), 
    by = c("pid", "visit_code")
  )
...
exposures <- 
  ...
  mutate(study_product = case_when(
    ...
    pid %in% pid_temp & !(visit_code %in% c(1101:1107)) ~ "0",

    pid == "068100013" & visit_code %in% c(1101:1107) ~ "1", 
    pid == "068100006" & visit_code %in% c(1301, 1401:1407) ~ "0",
    pid == "068100053" & visit_code %in% c(1101:1102, 1104:1107) ~ "0",
    pid == "068100053" & visit_code == 1103 ~"1", 
    pid == "068100054" & visit_code %in% c(1202:1207, 1003,1004) ~"0",
    pid == "068100058" & visit_code == 1003 ~ "0",
    pid == "068100061" & visit_code %in% c(1000:1007, 1201:1207, 1301) ~ "0", 
...
exposures |>
  ggplot(aes(y = factor(visit_code), x = factor(pid), color = study_product)) +
  geom_point() +
  scale_color_discrete(na.translate = FALSE) +  
  scale_color_manual(
