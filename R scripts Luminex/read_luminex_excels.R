read_luminex_excels <- function(
  ...
  if (length(files) == 0) {
    stop("No excel files found in the directories")
  }
...
read_luminex_excel <- function(file) {
  ...
  if (!all(expected_sheets %in% excel_sheets(path = file))) {
    stop(str_c(
      "The excel file does not contain one of the expected sheets: ",
      expected_sheets |> str_c(collapse = ", ")
    ))
  }
  ...
  sample_info_data_range <- str_c(
    "A",
    lc$top_line - 1,
    ":",
    get_letter(lc$first_column_num - 1),
    lc$bottom_line
  )
  sample_info <- read_excel(
    file,
    sheet = "FI",
    range = sample_info_data_range,
    col_names = TRUE
  )
  if (!("Well" %in% colnames(sample_info))) {
    stop("The sample information does not contain a 'Well' column")
  }
  if (!("Type" %in% colnames(sample_info))) {
    stop("The sample information does not contain a 'Type' column")
  }
  if (!("Description" %in% colnames(sample_info))) {
    warning("The sample information does not contain a 'Description' column")
  }
  ...
  FI <-
    read_excel(
      file,
      sheet = "FI",
      range = lc$excel_data_range,
      col_names = lc$analytes
    ) |>
    as.data.frame() |>
  ...
  data_range <- get_data_range_in_sheet(
    file,
    sheet = "FI - Bkgd",
    lc$excel_data_range,
    lc$analytes
  ...
  if (any(str_detect(readxl::excel_sheets(path = file), "Dil*ution"))) {
    ...
    if (d |> t() |> janitor::remove_constant() |> ncol() != 0) {
      stop(
        "The dilution values appear to be different for the same sample but different analytes\n"
      )
    }
    d <- tibble(
      well = sample_info$Well,
      dilution = d[, 1] |> unlist() |> str_replace(",", "\\.") |> as.numeric()
    )
  } else {
    d <- tibble(dilution = 1)
  }
...
identify_data_range <- function(file) {
  ...
  header_row <-
    read_excel(
      file,
      sheet = "FI",
      range = str_c("A", top_line - 2, ":BZ", top_line - 2),
  ...
  lc <- list(
    ...
    first_column_num = first_column,
    last_column_num = last_column,
    n_cols = last_column - first_column + 1,
    first_column = get_letter(first_column),
    last_column = get_letter(last_column),
    excel_data_range = str_c(
      get_letter(first_column),
      top_line,
      ":",
      get_letter(last_column),
      bottom_line
    ),
    analytes = analytes
  )
...
get_data_range_in_sheet <- function(
  file,
  sheet,
  data_range_in_FI,
  cytokine_names
  ...
  if (!check_cytokine_names(cytokine_names, file, sheet, data_range_in_FI)) {
    ...
    full_colnames <- read_excel(
      file,
      sheet = sheet,
      range = colnames_range,
      col_names = as.character(1:78)
    )
    matching_cols_num <- match(cytokine_names, full_colnames)
    if (
      any(is.na(matching_cols_num)) |
        (length(matching_cols_num) != length(cytokine_names))
    ) {
...
check_cytokine_names <- function(
  ...
  cytokine_names_check <- read_excel(
    file,
    sheet = sheet,
    range = cytokine_names_range
  ) |>
    colnames()
...
extract_lines_cols_from_range <- function(data_range) {
  ...
  first_col_num <- first_col |>
    str_split("") |>
    unlist() |>
    match(LETTERS) |>
    multiply_by(26^((str_length(first_col) - 1):0)) |>
    sum()
  last_col_num <- last_col |>
    str_split("") |>
    unlist() |>
    match(LETTERS) |>
    multiply_by(26^((str_length(last_col) - 1):0)) |>
    sum()
  ...
  list(
    ...
    last_col = last_col,
    n_cols = n_cols,
    first_col_num = first_col_num,
    last_col_num = last_col_num
  )
}
...
combine_se_coldata <- function(se_list) {
  map_dfr(
    ...
    ~ se_list[[.x]] |>
      ...
      mutate(
        plate_nb = .x,
        plate_name = names(se_list)[.x] |> basename() |> str_remove("\\.xlsx$")
      ) |>
      select(plate_nb, plate_name, everything()) |>
      set_rownames(str_c(
        str_pad(.x, width = 3, pad = "0"),
        "_",
        colnames(se_list[[.x]])
      ))
  ) |>
