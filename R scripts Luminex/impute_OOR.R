...
impute_OOR_deprecated <- function(
  se,
  max_log2_fc_exp_vs_obs = 1,
  LLOQ_factor = 0.5,
  ULOQ_factor = 1.1
) {
  # We first compute the lowest and highest extrapolated values from the samples concentrations
  ...
  standards <-
    ...
    mutate(
      obs_conc = raw_obs_conc |>
        str_replace(",", "\\.") |>
        str_remove("\\*") |>
        str_replace_all("OOR [<>]", "") |>
        as.numeric(), #
      obs_over_exp = obs_conc / exp_conc,
      obs_match_exp = !is.na(obs_conc) &
        (abs(log2(obs_over_exp)) < max_log2_fc_exp_vs_obs)
    ) |>
    group_by(plate_nb, .feature) |>
    mutate(
      LLOQ = min(exp_conc[obs_match_exp]),
      ULOQ = max(exp_conc[obs_match_exp]),
      standard_category = case_when(
        (exp_conc == LLOQ) ~ "LLOQ",
        (exp_conc == ULOQ) ~ "ULOQ",
  ...
  tmp <-
    ...
    mutate(
      ...
      value_type = case_when(
        ...
        str_detect(raw_obs_conc, "^\\*") &
          (obs_conc <= mid) ~ "Extrapolated (~ LLOQ)",
        str_detect(raw_obs_conc, "^\\*") &
          (obs_conc > mid) ~ "Extrapolated (~ ULOQ)",
