plot_standard_curves <- function(t) {
  t |>
    ...
    geom_hline(
      ...
      aes(yintercept = FI |> asinh(), col = plate_name),
      alpha = 0.5
    ) +
    ...
    labs(
      caption = "Horizontal lines show the FI for Blank samples on the same plate."
    )
}
...
plot_samples_on_standard_curves <- function(se, feature) {
  ...
  samples |>
    ...
    geom_hline(
      data = blanks,
      aes(yintercept = FI |> asinh()),
      alpha = 0.5
    ) +
    geom_point(
      data = standards,
      aes(x = exp_conc |> log10()),
      col = "green2",
      size = 2,
      alpha = 0.6
    ) +
    ...
    labs(
      title = feature,
      caption = str_c(
        "Green dots show Standards, for which the expected concentrations are shown on the x-axis.\n",
